...
class p5Manager {
    ...
    //====================================================
    #allStages = Object.values(DrawStages).filter(v => typeof v === "number");
    #userStages = [DrawStages.predraw, DrawStages.draw, DrawStages.postdraw];
    #userDrawCalls = new Map(this.#allStages.map(stage => [
        stage,
        []
    ]));
    registerDrawCall(func, stage = DrawStages.draw, top = false) {
        if (!isFunction(func))
    ...
    clearDrawCalls(stage = DrawStages.draw) {
        const stageCalls = this.#userDrawCalls.get(stage);
        if (stageCalls)
            stageCalls.length = 0;
    }
